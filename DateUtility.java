    ...
    public static int NextBusinessWorkingDay(Date date, int dayInterval) {
        ...
        while(!isWorkingDayFound){
            ...
            if(dow > LAST_DAY_WEEK && dow <= DAY_SUN){// to traverse the business day excluding weekend and starting from in between Monday (2) to Friday (6) 
                ...
                CommonMessages.logGenericTrace("NextBusinessWorkingDay() - are we here 1: ");
            
                dow -= LAST_DAY_WEEK; 
                CommonMessages.logGenericTrace("NextBusinessWorkingDay() - are we here 1.1: " + dow);
                ...
                if(dayOfWeek == Calendar.SATURDAY){
                    ...
                    dow -= 2;
                    CommonMessages.logGenericTrace("NextBusinessWorkingDay() - are we here 1.2: " + dow);
                }
            }
            else if(dow > DAY_SUN){
                // this will work if dow comes to value > 8
                CommonMessages.logGenericTrace("NextBusinessWorkingDay() - are we here 2: ");
                dow -= LAST_DAY_WEEK; 
            ...
            if( findBusinessDay(dow)){
                isWorkingDayFound = true;
                CommonMessages.logGenericTrace("NextBusinessWorkingDay() - 1.3 isWorkingDayFound is true, businessDayCounter: " + businessDayCounter);
